...
define_element!(
    ...
    /// The length of `line_a` is `difference` units longer than `line_b`.
...
impl AsConstraintData for LengthDifference {
    fn to_some_handle(handle: u32) -> SomeConstraintHandle {
        SomeConstraintHandle::LengthDifference(handle)
    }
...
mod tests {
    ...
    fn on_workplane() {
        ...
        let difference = 30.0;
        sys.constrain(LengthDifference::new(
            g,
            line_a,
            line_b,
            difference,
        ...
        if let (
            ...
            len_within_tolerance!(
                distance(a_start, a_end) - distance(b_start, b_end),
                difference
            );
        } else {
            unreachable!()
    ...
    fn in_3d() {
        ...
        let difference = 23.0;
        sys.constrain(LengthDifference::new(g, line_a, line_b, difference, None))
            .expect("constraint added");
